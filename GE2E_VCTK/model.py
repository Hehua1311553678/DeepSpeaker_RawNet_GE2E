...
def train(path):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for iter in range(config.iteration):
            ...
            train_times.append(str(begin_time)+'_'+str(end_time)+'_'+str(end_time-begin_time))# ===========================================================================2020/05/20 16:30
            ...
            if (iter+1) % 100 == 0:
                ...
                print("iter:{},耗时:{}s".format(iter, str(end_time - begin_time)))  # ===========================================================================2020/05/20 16:30
            if (iter+1) % 10000 == 0:
        ...
        # 将时间写入文件
        with open('GE2E_epoch说话人{}_batch说话人{}_人均音频数{}_iter{}_迭代耗时.txt'.format(config.spk_num, config.N, config.M, config.iteration), mode='w', encoding='utf-8') as wf:
            wf.write("epoch说话人{}个；batch说话人:{}个；人均音频数:{}条；迭代总次数：{}；平均每次训练迭代耗时:{}\n".format(
                config.spk_num, config.N, config.M, config.iteration, total_times/config.iteration))
