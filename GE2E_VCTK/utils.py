...
def random_batch(speaker_num=config.N, utter_num=config.M, shuffle=True, noise_filenum=None, utter_start=0):
    ...
    if config.tdsv:
        ...
        if config.train == True:
            # 修改说话人个数
            np_file_list = np_file_list[:config.spk_num]
        # ===========================================================结束
        ...
        for file in selected_files:
            ...
            if shuffle:
                ...
                utter_batch.append(np.concatenate(utters[utter_index], axis=1))  # each speakers utterance [M, n_mels, frames] is appended====================2020/04/01 21:42
            else:
                utter_batch.append(np.concatenate(utters[utter_start: utter_start + utter_num], axis=1))#====================2020/04/01 21:42

        utter_batch = np.concatenate(utter_batch, axis=1)  # utterance batch [batch(NM), n_mels, frames]#====================2020/04/01 21:42
        # ============================================================================================================================
        # ============================================================================================================================2020/04/11 11:28
